...
export function handleTransfer(event: Transfer): void {
  ...
  entity.date = new Date(entity.blockTime.toI64() * 1000).toDateString();
  let fromZeros = event.params.from == ZERO_ADDRESS;

  let totalRewardsEntity = TotalReward.load(event.transaction.hash);

  // Added metrics
  const user1Hash = `${entity.date}+_${entity.from}`;
  const user2Hash = `${entity.date}+_${entity.to}`;
  let dailySnap = DailySnap.load(entity.date);
  if (dailySnap == null) {
    let newEntry = new DailySnap(entity.date);
    newEntry.txcount = 1;
    newEntry.activeusers = 2;
    newEntry.save();
  } else {
    dailySnap.txcount = dailySnap.txcount + 1;
    let user_count = 0;
    if (UniqueUserHash.load(user1Hash) == null) {
      user_count += 1;
      new UniqueUserHash(user1Hash).save();
    }
    if (UniqueUserHash.load(user2Hash) == null) {
      user_count += 1;
      new UniqueUserHash(user2Hash).save();
    }
    dailySnap.activeusers += user_count;
    dailySnap.save();
  }
